...
public class CheckoutService {
     ...
     * Process checkout using Stripe payment.
     */
    public void processStripeCheckout(String customerName, String address, String stripeToken , HttpSession session) throws StripeException {

        // Perform Stripe payment
        performStripePayment(calculateCartTotal(getCartItems(session)), stripeToken);
    ...
    /**
     * Retrieves the current cart items from HttpSession.
     */
    private List<CartItem> getCartItems(HttpSession session) {
        // Returns the list of items in the session cart
